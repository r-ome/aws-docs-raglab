...
def insert_version(conn, doc_id, fetch_result: FetchResult, content_hash, raw_path, normalized_path, changed_from_previous):
    conn.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);
    """,
    [str(uuid4()), doc_id, fetch_result["url"], datetime.now(timezone.utc).isoformat(), fetch_result["status_code"], content_hash, raw_path, normalized_path, changed_from_previous])